          ...
          <div className="grid md:grid-cols-2 gap-16">
            ...
            <div className="border border-clay/20 p-12">
              ...
              <div className="mb-8">
                ...
                {(() => {
                  const huntingExperience = totals.dailyCost + formData.speciesFees;
                  const conservation = totals.conservationContribution;
                  const totalAllocation = huntingExperience + conservation;
                  const huntingPercent = totalAllocation > 0 ? (huntingExperience / totalAllocation) * 100 : 0;
                  const conservationPercent = totalAllocation > 0 ? (conservation / totalAllocation) * 100 : 0;
                  
                  return (
                    <div className="space-y-4">
                      {/* Visual Bar */}
                      <div className="h-8 bg-charcoal/50 border border-clay/20 overflow-hidden flex">
                        <div
                          className="bg-clay/30 border-r border-clay/40 flex items-center justify-center transition-all duration-500"
                          style={{ width: `${huntingPercent}%` }}
                        >
                          {huntingPercent > 15 && (
                            <span className="text-cream/80 font-body text-[10px] uppercase tracking-[0.1em] font-light px-2">
                              Experience
                            </span>
                          )}
                        </div>
                        <div
                          className="bg-clay/60 flex items-center justify-center transition-all duration-500"
                          style={{ width: `${conservationPercent}%` }}
                        >
                          {conservationPercent > 15 && (
                            <span className="text-cream font-body text-[10px] uppercase tracking-[0.1em] font-light px-2">
                              Conservation
                            </span>
                          )}
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-xs">
                        <div className="flex items-center gap-2">
                          <div className="w-3 h-3 bg-clay/30 border border-clay/40"></div>
                          <div>
                            <p className="text-silver/60 font-body tracking-[0.05em] font-light">Hunting Experience</p>
                            <p className="text-cream font-heading text-sm font-light">${huntingExperience.toLocaleString()}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="w-3 h-3 bg-clay/60"></div>
                          <div>
                            <p className="text-clay/80 font-body tracking-[0.05em] font-light">Conservation</p>
                            <p className="text-clay font-heading text-sm font-light">${conservation.toLocaleString()}</p>
                          </div>
                        </div>
                      </div>
                    </div>
